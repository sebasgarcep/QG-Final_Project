classdef SDAIterativeEnsembleKalmanSmootherN < Smoother
    ...
    methods
        function self = SDAIterativeEnsembleKalmanSmootherN(max_iter, epsilon, tol, lambda, U, L)
            self@Smoother(L);
        ...
        function [XP, CP] = update_smoother(self, X, C, Y, R, H, M, T)
            ...
            while true
                ...
                nabla_J = N * w / (self.epsilon + w' * w) - YL' / RL * (yL - yL_bar);
            ...
            H_tilde = N * ((self.epsilon + w' * w) * eye(N) - 2 * (w * w')) / (self.epsilon + w' * w).^2 + YL' / RL * YL;
